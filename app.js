// This file starts loading all the settings and paths
// And then calls function init.js in src folder
...
function main() {
  ...
  if (program.config) {                                             // if there is a config file specified in the command line arguments
    appConfigFile = path.resolve(process.cwd(), program.config);
  }
  var appConfig = JSON.parse(fs.readFileSync(appConfigFile, 'utf8'));  // Reading the configurations

  if (!appConfig.dapp.masterpassword) {                       // If there is no master password, making one
  ...
  // Proto file is the file specifying rules for messages
